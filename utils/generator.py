...
class DataGenerator(object):
    ...
    preprocessing_stategy = None

    PREPROCESSING_STRATEGIES = ["rows", "columns", "mean"]
    ...
    def reset(self):
        ...
        self.preprocessing_stategy = None
    ...
    def get_preprocessed_data(self, strategy=None):
        if self.preprocessed_data is None:
            if self.preprocessing_stategy is None:
                self.preprocessing_stategy = self.PREPROCESSING_STRATEGIES[0]
            elif strategy not in self.PREPROCESSING_STRATEGIES:
                raise ValueError("Unknown strategy", strategy)
            self.generate_preprocessed_data(self.preprocessing_stategy)
    ...
    def generate_preprocessed_data(self, strategy):
        if self.raw_data is None:
            self.generate_raw_data()

        df_X, df_y = self.raw_data

        if self.preprocessing_stategy not in self.PREPROCESSING_STRATEGIES:
            raise ValueError(f"Preprocessing strategy {self.preprocessing_stategy} unknown")

        if self.preprocessing_stategy == "rows":
            df_X = df_X.dropna()
            df_y = df_y.loc[df_X.index]
        elif self.preprocessing_stategy == "columns":
            df_X = df_X.dropna(axis=1)
        elif self.preprocessing_stategy == "mean":
            df_X.fillna(df_X.mean())
        else:
            raise NotImplementedError(
                f"Preprocessing strategy {self.preprocessing_stategy} not implemented")
